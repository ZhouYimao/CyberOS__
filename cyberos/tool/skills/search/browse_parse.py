...
def browse(url:str, options) :
    ...
    text = init_soup.get_text(separator=' ', strip=True)
    ...
    all_message = {'origin_url':url,'page_content':cleaned_text}
    all_message['link'] = links_and_descriptions
    driver.quit()  # 确保退出浏览器
    return all_message

async def browse_parse(urls: list):
...
if __name__ == "__main__":
    ...
    results = asyncio.run(browse_parse(urls))
